#include <Wire.h>
#include <LiquidCrystal_I2C.h>

LiquidCrystal_I2C lcd(0x27, 20, 4);

const int VRX = A0;
const int VRY = A1;

String morseBuffer = "";
String finalText = "";
String lastDisplayedText = "";

unsigned long lastInputTime = 0;
unsigned long lastMorseInput = 0;

const unsigned long idleLimit = 5000;
const unsigned long letterDelay = 700;

// Hold-clear
unsigned long holdStart = 0;

// Edge detection flags
bool upRegistered = false;
bool downRegistered = false;
bool leftRegistered = false;
bool rightRegistered = false;

// Idle trigger flag
bool idleExceeded = false;

// Morse dictionary
char morseToChar(String code) {
  if (code == ".-") return 'A';
  if (code == "-...") return 'B';
  if (code == "-.-.") return 'C';
  if (code == "-..") return 'D';
  if (code == ".") return 'E';
  if (code == "..-.") return 'F';
  if (code == "--.") return 'G';
  if (code == "....") return 'H';
  if (code == "..") return 'I';
  if (code == ".---") return 'J';
  if (code == "-.-") return 'K';
  if (code == ".-..") return 'L';
  if (code == "--") return 'M';
  if (code == "-.") return 'N';
  if (code == "---") return 'O';
  if (code == ".--.") return 'P';
  if (code == "--.-") return 'Q';
  if (code == ".-.") return 'R';
  if (code == "...") return 'S';
  if (code == "-") return 'T';
  if (code == "..-") return 'U';
  if (code == "...-") return 'V';
  if (code == ".--") return 'W';
  if (code == "-..-") return 'X';
  if (code == "-.--") return 'Y';
  if (code == "--..") return 'Z';

  if (code == "-----") return '0';
  if (code == ".----") return '1';
  if (code == "..---") return '2';
  if (code == "...--") return '3';
  if (code == "....-") return '4';
  if (code == ".....") return '5';
  if (code == "-....") return '6';
  if (code == "--...") return '7';
  if (code == "---..") return '8';
  if (code == "----.") return '9';

  if (code == ".-.-.-") return '.';
  if (code == "--..--") return ',';
  if (code == "..--..") return '?';
  if (code == ".----.") return '\'';
  if (code == "-.-.--") return '!';
  if (code == "-..-.") return '/';
  if (code == "-.--.") return '(';
  if (code == "-.--.-") return ')';
  if (code == ".-...") return '&';
  if (code == "---...") return ':';
  if (code == "-.-.-.") return ';';
  if (code == "-...-") return '=';
  if (code == ".-.-.") return '+';
  if (code == "-....-") return '-';
  if (code == "..--.-") return '_';
  if (code == ".-..-.") return '\"';
  if (code == "...-..-") return '$';
  if (code == ".--.-.") return '@';

  return '?';
}

void printToLCD(String text) {
  lcd.clear();
  for (int row = 0; row < 4; row++) {
    int start = row * 20;
    if (start >= text.length()) break;
    int len = min(20, text.length() - start);
    lcd.setCursor(0, row);
    lcd.print(text.substring(start, start + len));
  }
}

void setup() {
  lcd.init();
  lcd.backlight();
  lcd.clear();
  lcd.print("Morse to Text");
  delay(1200);
  lcd.clear();
  lastInputTime = millis();
}

void loop() {

  int x = analogRead(VRX);
  int y = analogRead(VRY);

  bool inputDetected = false;

  // ---------- DOT (UP) ----------
  if (y < 300) {
    if (!upRegistered) {
      if (idleExceeded) {
        finalText = "";
        morseBuffer = "";
        lastDisplayedText = "";
        idleExceeded = false;
      }
      morseBuffer += ".";
      lastMorseInput = millis();
      upRegistered = true;
      inputDetected = true;
    }
  } else upRegistered = false;


  // ---------- SPACE (DOWN) ----------
  if (y > 700) {
    if (!downRegistered) {
      if (idleExceeded) {
        finalText = "";
        morseBuffer = "";
        lastDisplayedText = "";
        idleExceeded = false;
      }
      finalText += " ";
      downRegistered = true;
      inputDetected = true;
      delay(250);
    }
  } else downRegistered = false;


  // ----------------------------------------------------
  // ---------- RIGHT = DASH "-"  (NEW) -----------------
  // ----------------------------------------------------
  if (x > 700) {
    if (!rightRegistered) {
      if (idleExceeded) {
        finalText = "";
        morseBuffer = "";
        lastDisplayedText = "";
        idleExceeded = false;
      }
      morseBuffer += "-";
      rightRegistered = true;
      lastMorseInput = millis();
      inputDetected = true;
    }
  } else rightRegistered = false;


  // ----------------------------------------------------
  // ---------- LEFT = BACKSPACE / HOLD CLEAR (NEW) -----
  // ----------------------------------------------------
  if (x < 300) {
    if (!leftRegistered) {

      // single backspace
      if (morseBuffer.length() > 0)
        morseBuffer.remove(morseBuffer.length() - 1);
      else if (finalText.length() > 0)
        finalText.remove(finalText.length() - 1);

      leftRegistered = true;
      inputDetected = true;
      holdStart = millis();
      delay(250);

    } else {
      // hold-left = full clear
      if (millis() - holdStart >= 1000) {
        finalText = "";
        morseBuffer = "";
        lastDisplayedText = "";
        lcd.clear();
        leftRegistered = false;
        delay(300);
        return;
      }
    }
  } else leftRegistered = false;


  // ---------- AUTO DECODE ----------
  if (morseBuffer.length() > 0 &&
      millis() - lastMorseInput > letterDelay) {

    char decoded = morseToChar(morseBuffer);
    finalText += decoded;
    morseBuffer = "";
  }


  // ---------- Idle 5 sec trigger ----------
  if (millis() - lastInputTime > idleLimit) {
    idleExceeded = true;
  }

  if (inputDetected) {
    if (idleExceeded) {
      // Clear then continue processing input
      finalText = "";
      morseBuffer = "";
      lastDisplayedText = "";
      idleExceeded = false;
    }
    lastInputTime = millis();
  }


  // ---------- Update LCD ----------
  if (finalText != lastDisplayedText) {
    printToLCD(finalText);
    lastDisplayedText = finalText;
  }
}
